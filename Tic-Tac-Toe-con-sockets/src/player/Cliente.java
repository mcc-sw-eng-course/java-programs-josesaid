...
public class Cliente implements Runnable{
    ...
    //El puerto debe ser el mismo en el que escucha el server
    private int puerto = 2027;
    //Si estamos en nuestra misma maquina usamos localhost si no la ip de la maquina server
    ...
    public void run() {
        try{
            //Cuando conectamos con el server, este nos devuelve el turno de juego
            mensaje =  in.readUTF();
    ...
    public void enviarTurno(int f,int c){
        ...
        Si es el turno entonces mandamos un mensaje al server con los datos de la jugada que hicimos
        */
        try {
            if(turno){
                String  datos = "";
